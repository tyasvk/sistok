...
<template>
    ...
    <div v-if="isKeluarModalOpen" class="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4">
        ...
        <div class="relative bg-white w-full md:w-[500px] max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl flex flex-col transition-all">
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-gray-100">
                ...
                <button @click="isKeluarModalOpen = false" class="p-2.5 text-gray-400 hover:text-gray-600 bg-gray-50 hover:bg-gray-100 rounded-full transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>

            <div class="p-6 md:p-8">
                <form @submit.prevent="submitKeluar" class="space-y-5">
                    
                    <div>
                        <label class="block text-xs font-bold text-gray-500 tracking-wider uppercase mb-1.5">Pilih Barang dari Gudang <span class="text-red-500">*</span></label>
                        <select v-model="formKeluar.product_id" required class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl py-3.5 px-4 text-sm focus:ring-2 focus:ring-red-500 outline-none appearance-none transition">
                            <option value="" disabled>-- Pilih barang yang akan diambil --</option>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-500 tracking-wider uppercase mb-1.5">Jumlah Pengambilan <span class="text-red-500">*</span></label>
                        <input type="number" min="1" v-model="formKeluar.jumlah" required placeholder="Masukkan kuantitas jumlah" class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl py-3.5 px-4 text-sm focus:ring-2 focus:ring-red-500 outline-none transition">
                        <p v-if="formKeluar.errors.jumlah" class="text-red-500 text-xs mt-1.5 font-semibold">{{ formKeluar.errors.jumlah }}</p>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-500 tracking-wider uppercase mb-1.5">Tujuan / Diserahkan Kepada <span class="text-red-500">*</span></label>
                        <input type="text" v-model="formKeluar.tujuan" required placeholder="Cth: Bagian Kepegawaian / Nama Petugas" class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl py-3.5 px-4 text-sm focus:ring-2 focus:ring-red-500 outline-none transition">
                    </div>

                    <div class="pt-2">
                        <label class="flex items-start cursor-pointer bg-red-50/50 p-4 border border-red-100 rounded-2xl select-none">
                            ...
                            <span class="ml-3 text-xs md:text-sm text-gray-700 leading-tight">
                                <strong class="text-red-700 block mb-0.5 font-bold uppercase tracking-wide">Verifikasi Pimpinan</strong>
                                Saya mengonfirmasi bahwa pengeluaran barang ini telah disetujui oleh pimpinan/Kasi Lapas.
                            </span>
                        </label>
                    ...
                    <button type="submit" :disabled="formKeluar.processing" class="w-full mt-4 py-4 bg-red-500 text-white rounded-xl font-bold text-base shadow-lg shadow-red-500/20 hover:bg-red-600 transition flex items-center justify-center" :class="{'opacity-70 cursor-not-allowed': formKeluar.processing}">
                        ...
                        {{ formKeluar.processing ? 'Memproses...' : 'Keluarkan Barang' }}
                    </button>
                </form>
            </div>
        </div>
